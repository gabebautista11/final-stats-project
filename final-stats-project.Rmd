...
<h1> Analysis <h1/>
In order to analyze our data, we should first look at the overall statistics of all three parking lots combined and we can infer some interesting things about our measurements
```{r}
...
```
Looking at the amount of car makes, we can see that from our sample, toyotas are the most popular choice of char, followed by subarus then hondas. Although it does not completley help us answer the question, it shows what cars are popular at the University of Utah and it also shows that since we can not measure the same amount of cars in each parking lot, our statistics may have some bias. For example, only one lexus was measured; thus, the mean for that brand in this study is based off of that one car, compared to the toyotas sample which has many cars measured and can give us a more accurate mean.
